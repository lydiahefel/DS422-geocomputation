...
# Make sure your chr_data is loaded
# glimpse(chr_data)

# Get county geometries
counties_sf <- counties(cb = TRUE, class = "sf")

# Join chr_data with county geometries using FIPS code
# Ensure FIPS codes are character and zero-padded to 5 digits
chr_data <- chr_data %>%
...
ggplot(map_data) +
  ...
  scale_fill_viridis_c(option = "plasma", na.value = "gray90", name = "% Fair/Poor Health") +
  theme_minimal() +
  labs(title = "Percent in Fair or Poor Health by County",
...
```{r}
#| label: top-feature-dependence
#| fig-cap: "Relationship between the top features and SHAP values for health outcomes"
