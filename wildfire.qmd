...
```{r}
wildfire_data <- wildfire_shp_data %>%
  group_by(ACQ_DATE) %>%
  summarise(total_fires = n())
```

```{r}
ggplot(wildfire_data, aes(x = ACQ_DATE, y = total_fires)) +
  geom_line() +
  labs(
    title = "Number of Wildfires Over Eight Days",
    x = "Date",
    y = "Number of Fires"
  ) +
  theme_minimal()
...
```{r}
more_wildfire_data <- wildfire_shp_data %>%
  group_by(ACQ_DATE, CONFIDENCE) %>%
  summarise(total_fires = n(), .groups = 'drop')

ggplot(more_wildfire_data, aes(x = ACQ_DATE, y = total_fires, fill = CONFIDENCE)) +
  geom_bar(stat = "identity") +
  labs(
    title = "Number of Wildfires Over Eight Days by Confidence Level",
    x = "Date",
    y = "Number of Fires",
    fill = "Confidence"
  ) +
...
wildfire_area_data <- st_read(
  "data/fire_kml/LANDSAT_USA_contiguous_and_Hawaii_7d_1757637791182.kml",
...
```{r}
conterminous_data <- read.csv("data/LANDSAT_USA_contiguous_and_Hawaii_7d.csv")

new_conterminous_data <- st_as_sf(conterminous_data, 
                                  coords = c("longitude", "latitude"), 
                                  crs = st_crs(4326))

leaflet(new_conterminous_data) |>
  addProviderTiles("CartoDB.Positron") |> 
  addCircleMarkers(
    radius = 5,
    color = ~colorFactor(viridis::viridis(4), confidence)(confidence),
    stroke = FALSE,
    fillOpacity = 0.8,
    popup = ~paste(
  ...
  addLegend(
    ...
    pal = colorFactor(viridis::viridis(4), new_conterminous_data$confidence),
    values = ~confidence,
